...
const KendoReactGridView = (props) => {
  ...
  const renderEmailCell = (props) => {
    ...
    return (
      <td>
        <Link className="email" to={{ pathname: `/contact/${props.dataItem.id}` }}>{email}</Link>
      </td>
    );
  };
  const renderPasswordCell = (props) => {
    const { password } = props.dataItem;
    return (
      <td>
        <Link className="email" to={{ pathname: `/contact/${props.dataItem.id}` }}>{password}</Link>
      </td>
    );
  };
  ...
  const renderActionCell = (props) => {
    ...
    return (
      <td >
        ...
        <button style={{ color: "red" }} onClick={() => deleteContactHandler(id)}>
          Delete
        </button>
      </td>
    );
